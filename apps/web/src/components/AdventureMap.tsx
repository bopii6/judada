  ...
  return (
    <div className="rounded-[2.5rem] bg-white p-8 sm:p-10 shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-100 relative overflow-hidden">
      ...
      <div className="flex items-center gap-3 mb-8">
        ...
        <div>
          <h2 className="text-xl font-black text-slate-800">冒险地图</h2>
          <p className="text-sm text-slate-500 font-medium">按单元学习，循序渐进征服英语世界！</p>
        </div>
      </div>
      ...
      <div className="space-y-4">
        {unitGroups.map((group, groupIndex) => {
          ...
          return (
            <div key={group.unitNumber ?? 'default'} className="border border-slate-100 rounded-2xl overflow-hidden">
              {/* Unit Header */}
              <button
                onClick={() => toggleUnit(group.unitNumber)}
                className="w-full flex items-center gap-4 p-4 bg-slate-50 hover:bg-slate-100 transition-colors text-left"
              >
                <div className={`p-2 rounded-xl ${unitProgress.percentage === 100 ? 'bg-emerald-100 text-emerald-600' : 'bg-indigo-100 text-indigo-600'}`}>
                ...
                <div className="hidden sm:flex items-center gap-3 w-32">
                  ...
                  <span className="text-xs font-bold text-slate-500 w-8">{unitProgress.percentage}%</span>
                </div>
              </button>
...
const StageCard = ({ stage, completed, unlocked, stars, showArrow, onStart, compact }: StageCardProps) => (
  ...
  >
    ...
    >
      ...
      {/* Stars Badge */}
      {stars > 0 && (
        <div className="absolute -bottom-1 bg-white px-1.5 py-0.5 rounded-full shadow-sm border border-slate-100 flex gap-0.5">
          {[...Array(stars)].map((_, i) => (
