...
export const WordHoverCard: React.FC<WordHoverCardProps> = ({ word, children, onClick }) => {
    ...
    return (
        ...
        >
            ...
            <span
                onClick={onClick}
                className="cursor-pointer hover:text-indigo-600 hover:scale-105 transition-all duration-200 active:scale-95 inline-block"
            >
                {children}
            </span>
            ...
            {showTooltip && (
                <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-3 z-50 w-64 animate-in fade-in zoom-in-95 duration-200">
                    <div className="bg-white/95 backdrop-blur-xl rounded-2xl shadow-xl shadow-indigo-500/10 border border-indigo-50 p-4 relative">
                        {/* Arrow */}
                        <div className="absolute -bottom-2 left-1/2 -translate-x-1/2 w-4 h-4 bg-white rotate-45 border-b border-r border-indigo-50" />

                        {isLoading ? (
                            <div className="flex items-center justify-center py-2 gap-2 text-indigo-400">
